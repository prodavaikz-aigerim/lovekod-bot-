...
OTHER_PERSON_PRICE = os.environ.get("OTHER_PERSON_PRICE", "1 600 ₸")
...
async def show_lovelang_result(query, context: ContextTypes.DEFAULT_TYPE, scores: dict):
    ...
    вдохновляющая история про пару, которая поняла общие мотиваторы
    (format_couple_success_story) — и только после неё оффер с выбором
    оплаты, чтобы история подводила к предложению, а не шла последней.
    """
...
async def start_male_test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запускается кнопкой (callback) — либо из выбора после языка любви,
    либо из CTA после отчёта."""
...
- Раздел 2 ("Мой язык любви") — помимо описания самого языка, дай 2-3 готовые короткие фразы, которыми она может прямо попросить партнёра о том, что ей нужно на её языке любви (например, если её язык — слова поддержки, дай готовую фразу-просьбу услышать эти слова; если это время вместе — готовую фразу-предложение).
